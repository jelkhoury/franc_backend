...
public class BlobStorageService
{
        ...
        IOptions<UploadSecurityOptions> uploadOptions)
    {
        ...
        // Production (Azure App Settings): BlobContainerName
        // Development (appsettings): AzureBlobStorage:ContainerName
        _container = env.IsProduction()
        ...
        if (string.IsNullOrWhiteSpace(_container))
        {
            throw new InvalidOperationException(
                env.IsProduction()
                    ? "Configuration key 'BlobContainerName' is required in Production."
                    : "Configuration key 'AzureBlobStorage:ContainerName' is required in Development.");
        }
        ...
        MockInterviewUploadDto dto)
    {
        ...
        if (dto.Videos.Count > _uploadOptions.MaxVideosPerMockInterview)
            throw new ArgumentException($"A maximum of {_uploadOptions.MaxVideosPerMockInterview} videos is allowed per mock interview.");

        await _uploadSecurity.ValidateBatchAsync(
            dto.Videos,
            FileUploadCategory.Video,
            _uploadOptions.MaxMockInterviewBatchBytes);
    ...
    public async Task<List<Question>> CreateQuestionsForMajorAsync(string majorName, List<IFormFile> videos)
    {
        ...
        await _uploadSecurity.ValidateBatchAsync(videos, FileUploadCategory.Video);
    ...
    public async Task<Question> CreateQuestionForMajorAsync(string majorName, string title, IFormFile video)
    {
        ...
        await _uploadSecurity.ValidateAsync(video, FileUploadCategory.Video);
    ...
    public async Task<Major> CreateMajorAsync(string name, string description, IFormFile image, int facultyId)
    {
        ...
        await _uploadSecurity.ValidateAsync(image, FileUploadCategory.Image);
      ...
      string? aiEvaluation = null)
    {
        ...
        async Task<string?> UploadToBlobAsync(IFormFile? file)
        {
            ...
            await _uploadSecurity.ValidateAsync(file, FileUploadCategory.Document);

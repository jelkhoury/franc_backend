...
public class SdsRepository : ISdsRepository
{
	...
	public async Task<int> CreateQuestionWithOptionsAsync(CreateSDSQuestionWithOptionsDto dto)
	{
		...
		if (dto.Type != QuestionType.TextBox)
		{
			...
			{
				var value = string.IsNullOrWhiteSpace(opt.Value) ? null : opt.Value.Trim().ToUpperInvariant();
    ...
    public async Task<object> SaveUserResponsesAsync(SubmitSDSResponsesDto dto)
    {
        ...
        if (dto.UserId <= 0) throw new ArgumentException("UserId is required.", nameof(dto));
        if (dto.Responses == null || dto.Responses.Count == 0)
            throw new ArgumentException("At least one response is required.", nameof(dto));

        // Get last attempt number for this user - FIXED VERSION
        ...
        {
            var qType = questions[r.QuestionId];
            ...
            entities.Add(new SDSResponse
            {
                ...
                SelectedValue = string.IsNullOrWhiteSpace(r.SelectedValue) ? null : r.SelectedValue.Trim().ToUpperInvariant(),
                CustomAnswer = string.IsNullOrWhiteSpace(r.CustomAnswer) ? null : r.CustomAnswer.Trim(),
                SubmittedAt = now
            });
        }
        ...
        // Recalculate Holland code based on the new attempt
        var hollandCode = await CalculateHollandCodeAsync(dto.UserId, newAttempt);

        // Get responses for specific questions in the new attempt
        var wantedIds = new[] { 362, 363 };
        var wantedResponses = await _context.SDSResponses
            .AsNoTracking()
            .Where(r => r.UserId == dto.UserId && r.AttemptNumber == newAttempt && wantedIds.Contains(r.QuestionId))
            .Select(r => new
            {
                r.QuestionId,
                r.SelectedValue,
    ...
    {
        // Define valid Holland codes
        var validCodes = new HashSet<string> { "R", "I", "A", "S", "E", "C" };

        // Get the last attempt number for this user
        var lastAttempt = await _context.SDSResponses
            .Where(r => r.UserId == userId)
            .MaxAsync(r => (int?)r.AttemptNumber) ?? 0;

        if (lastAttempt == 0)
        {
            // No attempts found, return empty result
            return new Dictionary<string, Dictionary<string, int>>(StringComparer.OrdinalIgnoreCase);
        }

        // Get all responses for the user's last attempt only, including question and section
        var responses = await _context.SDSResponses
            .AsNoTracking()
            .Where(r => r.UserId == userId && r.AttemptNumber == lastAttempt && !string.IsNullOrWhiteSpace(r.SelectedValue))
            .Include(r => r.Question)
                .ThenInclude(q => q.Section)
            .ToListAsync();

        // Initialize result dictionary
        // Key = SectionName, Value = Dictionary<HollandCode, Points>
        ...
        foreach (var r in responses)
        {
            ...
            {
                // Initialize points for each code in this section
                result[sectionName] = validCodes.ToDictionary(c => c, c => 0);
            }

            var value = r.SelectedValue.Trim().ToUpper();

            // Extract code letter (last char)
            var codeLetter = value[^1].ToString();
            if (!validCodes.Contains(codeLetter))
                continue;

            // Extract number prefix
            var numberPart = value.Length > 1 ? value.Substring(0, value.Length - 1) : "";
            int score = 1;
            if (!string.IsNullOrWhiteSpace(numberPart) && int.TryParse(numberPart, out int parsed))
                score = parsed;

            // Add score to the corresponding code in the section
	...
	IEnumerable<CreateSDSQuestionWithOptionsDto> dtos)
	{
		...
		if (dtoList.Count == 0) return Array.Empty<int>();
		...
		for (int i = 0; i < dtoList.Count; i++)
		{
			...
			{
				if (dto.AnswerOptions == null || dto.AnswerOptions.Count == 0)
					throw new InvalidOperationException($"Non-TextBox question at index {i} must include answer options.");
			}
		}
    ...
    public async Task<string> CalculateHollandCodeAsync(int userId, int attemptNumber)
    {
        ...
        // Only calculate based on the specific attempt
        var responses = await _context.SDSResponses
            .Where(r => r.UserId == userId
        ...
        foreach (var val in responses)
        {
            ...
            var numberPart = val.Length > 1 ? val.Substring(0, val.Length - 1) : "";
            if (!int.TryParse(numberPart, out int score))
                score = 1;
